...
if __name__ == "__main__":
    ...
    # # # 可参考demo
    # # data_path = norm_csv
    data_path = "dataset/120000/dofbot_fk_120000_norm.csv"
    stats_path = "dataset/120000/dofbot_fk_120000_norm_stats.json"
    ...
    fk_model, fk_dir, fk_path = train_dofbot_model(
        ...
        epochs=fk_epochs, lr=fk_lr, min_lr=1e-5, hidden_layers=fk_hidden_layers, w_pos=0.95, w_ori=0.05
    )
    ...
    ik_model, ik_dir, ik_path = train_dofbot_model(
        ...
        fk_path=fk_path, fk_hidden_layers=fk_hidden_layers, w_pos=0.95, w_ori=0.05
    )
    ...
    # 保存FK预测/真值/误差到CSV（便于任务六报告）
    os.makedirs('results/model_results', exist_ok=True)
    pose_cols = ['x','y','z','nx','ny','nz','ox','oy','oz','ax','ay','az']
    pd.DataFrame(fk_res['nn_pose'], columns=pose_cols).to_csv('results/model_results/fk_pred_pose.csv', index=False)
    pd.DataFrame(fk_res['pb_pose'], columns=pose_cols).to_csv('results/model_results/fk_gt_pose.csv', index=False)
    pd.DataFrame(fk_res['err'][:, :3] * 1000, columns=['ex_mm','ey_mm','ez_mm']).to_csv('results/model_results/fk_err_mm.csv', index=False)
    ...
    # 保存IK目标/仿真真值/误差/预测关节角到CSV
    pd.DataFrame(ik_res['tgt_arr'], columns=pose_cols).to_csv('results/model_results/ik_target_pose.csv', index=False)
    pd.DataFrame(ik_res['pb_pose'], columns=pose_cols).to_csv('results/model_results/ik_pb_pose.csv', index=False)
    pd.DataFrame(ik_res['err'][:, :3] * 1000, columns=['ex_mm','ey_mm','ez_mm']).to_csv('results/model_results/ik_err_mm.csv', index=False)
    pd.DataFrame(ik_res['q_pred'], columns=['q1_deg','q2_deg','q3_deg','q4_deg','q5_deg']).to_csv('results/model_results/ik_pred_joints.csv', index=False)
