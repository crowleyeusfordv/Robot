...
class UmbrellaGameDriver(object):
    def __init__(self):
        self.rate_hz = rospy.get_param('~rate_hz', 75.0)
        self.mass = rospy.get_param('~mass', 1.0)
        self.damping = rospy.get_param('~damping', 0.18)
        self.pursuer_max_speed = rospy.get_param('~pursuer_max_speed', 4.15)
        self.evader_max_speed = rospy.get_param('~evader_max_speed', 3.05)
        self.z_height = rospy.get_param('~z_height', 0.12)

        self.states = {
            'pursuer_umbrella': VehicleState(1.88, 0.34),
            'evader_umbrella': VehicleState(2.68, 2.55),
        }

        self.set_model_pub = rospy.Publisher('/gazebo/set_model_state', ModelState, queue_size=20)
        self.state_pub = rospy.Publisher('/umbrella/model_states', ModelStates, queue_size=10)

        rospy.Subscriber('/umbrella/pursuer/control', Twist, self.pursuer_callback)
        rospy.Subscriber('/umbrella/evader/control', Twist, self.evader_callback)

        self.last_time = rospy.get_time()
        self.timer = rospy.Timer(rospy.Duration(1.0 / self.rate_hz), self.step)

    def pursuer_callback(self, msg):
        self.set_control('pursuer_umbrella', msg)

    def evader_callback(self, msg):
        self.set_control('evader_umbrella', msg)
    ...
    def step(self, _event):
        now = rospy.get_time()
        dt = max(0.0, min(now - self.last_time, 0.065))
        self.last_time = now
    ...
    def integrate(self, name, state, dt):
        ...
        max_speed = self.pursuer_max_speed if name == 'pursuer_umbrella' else self.evader_max_speed
    ...
    def publish_combined_state(self):
        msg = ModelStates()
        for name in ('pursuer_umbrella', 'evader_umbrella'):
            state = self.states[name]
...
if __name__ == '__main__':
    try:
        rospy.init_node('driver_umbrella', anonymous=True)
        UmbrellaGameDriver()
        rospy.spin()
